...
'''
Test script
*WARNING* Don't run this on a production bitbean server! *WARNING*
...
import bitbeanrpc
from bitbeanrpc.exceptions import bitbeanException, InsufficientFunds
...
if __name__ == "__main__":
    ...
    if args.config:
        from bitbeanrpc.config import read_config_file
    ...
    connections = []
    if not args.nolocal:
        local_conn = bitbeanrpc.connect_to_local()  # will use read_default_config
        connections.append(local_conn)
    if not args.noremote:
        remote_conn = bitbeanrpc.connect_to_remote(
    ...
    for conn in connections:
        ...
        bitbeanaddress = conn.getnewaddress(account)
        conn.setaccount(bitbeanaddress, account)
        ...
        #conn.sendtoaddress(bitbeanaddress, amount, comment=None, comment_to=None)
        conn.getreceivedbyaddress(bitbeanaddress)
        ...
        for message in messages:
            signature = conn.signmessage(bitbeanaddress, message)
            assert(conn.verifymessage(bitbeanaddress, signature, message) is True)
        ...
        try:
            conn.sendtoaddress(bitbeanaddress, 10000000)
